<?php
 ...
 $username = "stokk";
  ...
  try {
    ...
    $Fiyat11=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE siparis_id>0");
    $Fiyat11->execute();
  $FiyatYaz11= $Fiyat11->fetch(PDO::FETCH_ASSOC);

 $Fiyat1=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun1'");
 $Fiyat1->execute();
$FiyatYaz1= $Fiyat1->fetch(PDO::FETCH_ASSOC);


$Fiyat2=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun2'");
$Fiyat2->execute();
$FiyatYaz2= $Fiyat2->fetch(PDO::FETCH_ASSOC);


$Fiyat3=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun3'");
$Fiyat3->execute();
$FiyatYaz3= $Fiyat3->fetch(PDO::FETCH_ASSOC);


$Fiyat4=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun4'");
$Fiyat4->execute();
$FiyatYaz4= $Fiyat4->fetch(PDO::FETCH_ASSOC);


$Fiyat5=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun5'");
$Fiyat5->execute();
$FiyatYaz5= $Fiyat5->fetch(PDO::FETCH_ASSOC);


$Fiyat6=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun6'");
$Fiyat6->execute();
$FiyatYaz6= $Fiyat6->fetch(PDO::FETCH_ASSOC);


$Fiyat7=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun7'");
$Fiyat7->execute();
$FiyatYaz7= $Fiyat7->fetch(PDO::FETCH_ASSOC);


$Fiyat8=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun8'");
$Fiyat8->execute();
$FiyatYaz8= $Fiyat8->fetch(PDO::FETCH_ASSOC);


$Fiyat9=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun9'");
$Fiyat9->execute();
$FiyatYaz9= $Fiyat9->fetch(PDO::FETCH_ASSOC);


$Fiyat10=$db->prepare("SELECT SUM(sip_genel_toplam) AS stok_isim FROM siparis WHERE sip_alan='$urun10'");
$Fiyat10->execute();
...
function drawChart() {
  ...
  var options = {'title':'Personel Satış Grafiği  --> Seçilen tarih Aralığındaki Tüm Ürünlerden Edilen Toplam Gelir :  <?php  echo $FiyatYaz11['stok_isim']; ?> ₺', 'width':1000, 'height':650} ;
...
<meta charset="UTF-8">
  ...
  <body style="background-color:#f7faff">
    ...
    <div id="piechart" style="width: 900px; height: 500px;background-color:#f7faff"></div>
